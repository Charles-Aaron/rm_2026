...
condition_free::condition_free(const std::string& name, const BT::NodeConfiguration& config)
    : BT::ConditionNode(name, config) {
     ...
     sub_ = node_->create_subscription<customize_messages::msg::Remotedata>(
        ...
        [this](const customize_messages::msg::Remotedata::SharedPtr msg) {
           ...
           flag_ = (msg->fort_flag==1);  // 如果 operator_control 为 1，flag_ 为 true
         });
}

BT::PortsList condition_free::providedPorts() {
    return {};
}

BT::NodeStatus condition_free::tick() {
