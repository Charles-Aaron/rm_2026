...
action_fort::action_fort(const std::string& name, const BT::NodeConfiguration& config)
    ...
    sub_ = node_->create_subscription<customize_messages::msg::Remotedata>(
        "/remote_data", 10, [this](const customize_messages::msg::Remotedata::SharedPtr msg) {
            // 检查标志位
            if ( (msg->supply_flag==0)|| msg->operator_control||msg->enemy_flag) {
...
BT::PortsList action_fort::providedPorts() {
    return {};  // 不再使用端口
}

BT::NodeStatus action_fort::onStart() {
    is_goal_done_ = false;
...
BT::NodeStatus action_fort::onRunning() {
    if (interrupt_navigation_) {
        RCLCPP_INFO(node_->get_logger(), "Interrupted navigation due to remote data flag");
        return BT::NodeStatus::FAILURE;
     }
