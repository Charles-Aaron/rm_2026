...
action_free::action_free(const std::string& name, const BT::NodeConfiguration& config)
    ...
    sub_ = node_->create_subscription<customize_messages::msg::Remotedata>(
        "/remote_data", 10, [this](const customize_messages::msg::Remotedata::SharedPtr msg) {
            // 检查标志位
            if (msg->supply_flag || (msg->fort_flag==0)|| msg->operator_control||msg->enemy_flag) {
    ...
    // 目标点1
     goals_[0].pose.position.x = 4.33;
     goals_[0].pose.position.y = 1.8;
...
BT::PortsList action_free::providedPorts() {
    return {};  // 不再使用端口
}

BT::NodeStatus action_free::onStart() {
    is_goal_done_ = false;
...
BT::NodeStatus action_free::onRunning() {
    if (interrupt_navigation_) {
        RCLCPP_INFO(node_->get_logger(), "Interrupted navigation due to remote data flag");
        return BT::NodeStatus::FAILURE;
     }
