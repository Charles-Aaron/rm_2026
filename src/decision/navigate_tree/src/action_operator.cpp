...
action_operator::action_operator(const std::string& name, const BT::NodeConfiguration& config)
    ...
    sub_ = node_->create_subscription<customize_messages::msg::Remotedata>(
        "/remote_data", 10, [this](const customize_messages::msg::Remotedata::SharedPtr msg) {
            ...
            current_goal_.pose.position.x = msg->map_x;
            current_goal_.pose.position.y = msg->map_y;
...
BT::PortsList action_operator::providedPorts() {
    return {};  // 不再使用端口
}

BT::NodeStatus action_operator::onStart() {
    is_goal_done_ = false;
...
BT::NodeStatus action_operator::onRunning() {
