...
action_start35::action_start35(const std::string& name, const BT::NodeConfiguration& config)
    ...
    sub_ = node_->create_subscription<customize_messages::msg::Remotedata>(
        "/remote_data", 10, [this](const customize_messages::msg::Remotedata::SharedPtr msg) {
            // 从消息中获取敌人距离和角度
            if ( msg->supply_flag|| msg->fort_flag|| msg->operator_control||msg->home_flag||msg->random_flag||msg->enemy_flag||msg->protect_flag==0) {
    ...
    // 目标点1
    goals_[0].pose.position.x = 7.3;
    goals_[0].pose.position.y = 5.6;
    goals_[0].pose.orientation.w = 1.0;

    // 目标点2
    goals_[1].pose.position.x = 12.8;
    goals_[1].pose.position.y = 7.51;
...
BT::PortsList action_start35::providedPorts() {
    return {};  // 不再使用端口
}

BT::NodeStatus action_start35::onStart() {
    is_goal_done_ = false;
...
BT::NodeStatus action_start35::onRunning() {
    if (interrupt_navigation_) {
        RCLCPP_INFO(node_->get_logger(), "Interrupted navigation due to remote data flag");
        return BT::NodeStatus::FAILURE;
     }
