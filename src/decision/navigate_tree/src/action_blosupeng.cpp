...
action_blosupeng:: action_blosupeng(const std::string& name, const BT::NodeConfiguration& config)
    ...
    // 目标点1补血点
    goals_[0].pose.position.x = 0.212;
    goals_[0].pose.position.y = -3.27;
    goals_[0].pose.orientation.w = 1.0;

    // // 目标点2补给点
    // goals_[1].pose.position.x = 0.0;
    // goals_[1].pose.position.y = -3.0;
    ...
    sub_ = node_->create_subscription<customize_messages::msg::Remotedata>(
        "/remote_data", 10, [this](const customize_messages::msg::Remotedata::SharedPtr msg) {
            // 检查标志位
            // blood_flag = msg->blood_return;       // 血包状态
            //supply_flag = msg->supply_flag;     // 补给点状态
            if (msg->operator_control||msg->enemy_flag) {
...
BT::PortsList  action_blosupeng::providedPorts() {
    return {};  // 不再使用端口
}

BT::NodeStatus  action_blosupeng::onStart() {
    is_goal_done_ = false;
...
BT::NodeStatus action_blosupeng::onRunning() {
    // 检查是否有其他任务，有的话直接打断巡航任务
    if (interrupt_navigation_) {
        RCLCPP_INFO(node_->get_logger(), "Interrupted navigation due to remote data flag");
        //interrupt_navigation_ = false;
...
void  action_blosupeng::onHalted() {
    if (client_ && goal_handle_ && future_goal_handle_.valid()) {
        try {
            client_->async_cancel_goal(goal_handle_);
            RCLCPP_INFO(node_->get_logger(), "Navigation halted");
        } catch (const rclcpp_action::exceptions::UnknownGoalHandleError& e) {
            RCLCPP_WARN(node_->get_logger(), "Goal handle is not known to this client. It may have already been canceled or completed.");
        }
    }
    ...
    future_goal_handle_ = std::shared_future<rclcpp_action::ClientGoalHandle<NavigateToPose>::SharedPtr>();
}