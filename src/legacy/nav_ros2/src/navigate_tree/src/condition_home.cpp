...
condition_home::condition_home(const std::string& name, const BT::NodeConfiguration& config)
    : BT::ConditionNode(name, config) {
     ...
     sub_ = node_->create_subscription<customize_messages::msg::Remotedata>(
        ...
        [this](const customize_messages::msg::Remotedata::SharedPtr msg) {
           // 提取 start_flag 字段
           flag_ = (msg->home_flag == 1);  // 如果 start_flag 为 1，flag_ 为 true
         });
}

BT::PortsList condition_home::providedPorts() {
    return {};
}

BT::NodeStatus condition_home::tick() {
