...
action_tower::action_tower(const std::string& name, const BT::NodeConfiguration& config)
    ...
    sub_ = node_->create_subscription<customize_messages::msg::Remotedata>(
        "/remote_data", 10, [this](const customize_messages::msg::Remotedata::SharedPtr msg) {
            // 检查标志位
            if ( msg->supply_flag || msg->protect_flag || msg->fort_flag|| msg->operator_control||msg->home_flag||msg->random_flag||msg->enemy_flag) {
    ...
    // 目标点1
    goals_[0].pose.position.x = 4.5;
    goals_[0].pose.position.y = 0.0;                              ;
    goals_[0].pose.orientation.w = 1.0;

    //目标点2
    goals_[1].pose.position.x = 2.11;
    goals_[1].pose.position.y = 1.57;
...
BT::PortsList action_tower::providedPorts() {
    return {};  // 不再使用端口
}

BT::NodeStatus action_tower::onStart() {
    is_goal_done_ = false;
...
BT::NodeStatus action_tower::onRunning() {
