...
patrol::patrol(const std::string& name, const BT::NodeConfiguration& config)
    ...
    goals_.resize(3);
    // 目标点1
    goals_[0].pose.position.x = 2.0;
    goals_[0].pose.position.y = 2.0;
    goals_[0].pose.orientation.w = 1.0;

    // 目标点2
    goals_[1].pose.position.x = -2.0;
    goals_[1].pose.position.y = -2.0;
    goals_[1].pose.orientation.w = 1.0;

    // 目标点3
    goals_[2].pose.position.x = 0.0;
    goals_[2].pose.position.y = 0.0;
    goals_[2].pose.orientation.w = 1.0;
    ...
    sub_ = node_->create_subscription<customize_messages::msg::Remotedata>(
        "/remote_data", 10, [this](const customize_messages::msg::Remotedata::SharedPtr msg) {
            // 检查标志位
            if (msg->tower_flag || msg->supply || msg->blood_return || msg->engineering ||
                msg->start_35_flag || msg->outpost_smash || msg->operator_control ||
                msg->enemy_flag) {
        ...
        //     RCLCPP_INFO(node_->get_logger(), "Reached tunnel_goal1!");
        // }
        });
}
...
BT::NodeStatus patrol::onRunning() {
    ...
    if (is_goal_done_) {
        ...
        if (is_goal_success_) {
            if (current_goal_index_ < 2) { //当一个点导航成功时进入下一个点进行导航点切换
                current_goal_index_++;
            } else {
