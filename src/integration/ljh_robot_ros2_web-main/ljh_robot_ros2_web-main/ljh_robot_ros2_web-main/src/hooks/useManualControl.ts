...
export function useManualControl(
  ...
  useEffect(() => {
    ...
    const parseSpeed = (value: unknown, fallback: number): number => {
    ...
    const updateCmdVel = () => {
      ...
      const cmdVelConfig = Object.values(layerConfigsRef.current).find((config) => config.id === 'cmd_vel');
      const linearXSpeed = parseSpeed(cmdVelConfig?.linearXSpeed, 0.5);
      const linearYSpeed = parseSpeed(cmdVelConfig?.linearYSpeed, 0.5);
      const angularZSpeed = parseSpeed(cmdVelConfig?.angularZSpeed, 0.5);
      ...
      if (hasA || hasLeft) {
        angular = angularZSpeed;
      }
      if (hasD || hasRight) {
        angular = -angularZSpeed;
      }
      if (hasZ) {
        linearY = linearYSpeed;
      }
      if (hasX) {
        linearY = -linearYSpeed;
      }
  ...
  }, [manualControlMode, connection, cmdVelTopicRef, layerConfigsRef, activeKeysRef, cmdVelIntervalRef]);
}
