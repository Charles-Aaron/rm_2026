...
export class TopoLayer extends BaseLayer {
  ...
  update(message: unknown): void {
    let msg: TopologyMap | null = null;
    if (typeof message === 'string') {
      try {
        msg = JSON.parse(message) as TopologyMap;
      } catch {
        msg = null;
      }
    } else if (typeof message === 'object' && message !== null) {
      const messageObj = message as { data?: unknown; points?: unknown };
      if (typeof messageObj.data === 'string') {
        try {
          msg = JSON.parse(messageObj.data) as TopologyMap;
        } catch {
          msg = null;
        }
      } else if (Array.isArray(messageObj.points)) {
        msg = messageObj as TopologyMap;
      }
    }
