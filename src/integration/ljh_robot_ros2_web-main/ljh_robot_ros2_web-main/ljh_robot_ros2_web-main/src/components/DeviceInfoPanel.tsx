...
import { toDeviceNavLabel } from '../utils/navStatus';
...
const ODOMETRY_TOPIC = withRobotNamespace('/odometry') ?? '/odometry';
const CMD_VEL_TOPIC = withRobotNamespace('/cmd_vel') ?? '/cmd_vel';
const BATTERY_TOPIC = withRobotNamespace('/battery_state') ?? '/battery_state';
const NAV_STATUS_TOPIC = withRobotNamespace('/web/nav_status') ?? '/web/nav_status';
const NAV_CANCEL_TOPIC = withRobotNamespace('/web/nav_cancel') ?? '/web/nav_cancel';
const EMERGENCY_STOP_TOPIC = withRobotNamespace('/robot/emergency_stop') ?? '/robot/emergency_stop';
...
export function DeviceInfoPanel({
  ...
  useEffect(() => {
      ...
      connection.subscribe(
        ...
        (message: any) => {
          if (message.status_list && message.status_list.length > 0) {
            ...
            const navStatus = toDeviceNavLabel(status);
    ...
    const updatePositionFromTF = () => {
      ...
      const transform = tf2js.findTransform('map', 'base_footprint');  // rm_2025: base_footprint
      if (transform) {
        const euler = new THREE.Euler();
    ...
    return () => {
      try {
        connection.unsubscribe(ODOMETRY_TOPIC);
        connection.unsubscribe(BATTERY_TOPIC);
        connection.unsubscribe(NAV_STATUS_TOPIC);
      } catch (error) {
        // Ignore
      }
  ...
  const handleStop = () => {
    ...
    try {
      // 停止运动
      connection.publish(CMD_VEL_TOPIC, 'geometry_msgs/msg/Twist', {
        linear: { x: 0, y: 0, z: 0 },
        angular: { x: 0, y: 0, z: 0 },
      });
      ...
      // 取消导航目标
      try {
        connection.publish(NAV_CANCEL_TOPIC, 'std_msgs/msg/Empty', {});
      } catch (error) {
  ...
  const handleEmergencyStop = () => {
    ...
    if (window.confirm('确定要执行急停吗？这将立即停止所有运动！')) {
      try {
        ...
        // 发布急停标志
        try {
          connection.publish(EMERGENCY_STOP_TOPIC, 'std_msgs/msg/Bool', { data: true });
        } catch (error) {
          // Ignore
        }
  ...
  return (
    <div className="DeviceInfoPanel">
      ...
      <div className="BrandLogo">
        ...
        <span className="BrandText">CHARLES</span>
      </div>
      ...
      <div className="DeviceImageContainer">
        ...
        />
        <div className="DeviceModel">NCU哨兵</div>
      </div>

      {/* 设备名称和在线状态 */}
      <div className="DeviceHeader">
        <div className="DeviceName">NCU哨兵</div>
