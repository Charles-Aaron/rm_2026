...
const MAP_TOPIC = withRobotNamespace('/map') ?? '/map';
const TOPOLOGY_TOPIC = withRobotNamespace('/web/topology') ?? '/web/topology';
const TOPOLOGY_UPDATE_TOPIC = withRobotNamespace('/web/topology/update') ?? '/web/topology/update';
...
function parseTopologyMessage(message: unknown): TopologyMap | null {
  if (!message) {
    return null;
  }
  ...
  if (typeof message === 'string') {
    try {
      return JSON.parse(message) as TopologyMap;
    } catch {
      return null;
    }
  }
  ...
  if (typeof message === 'object') {
    const msg = message as { data?: unknown; points?: unknown };
    if (typeof msg.data === 'string') {
      try {
        return JSON.parse(msg.data) as TopologyMap;
      } catch {
        return null;
      }
    }
    if (Array.isArray(msg.points)) {
      return msg as TopologyMap;
    }
  }
...
export class MapManager {
  ...
  public disconnect(): void {
    if (this.connection && this.connection.isConnected()) {
      this.connection.unsubscribe(TOPOLOGY_TOPIC);
      this.connection.unsubscribe(MAP_TOPIC);
    }
  ...
  private subscribeToTopics(): void {
    ...
    const topologyCallback = (message: unknown) => {
      const msg = parseTopologyMessage(message);
      ...
      if (msg && msg.points && Array.isArray(msg.points)) {
    ...
    console.log('[MapManager] Subscribing to topology topic:', TOPOLOGY_TOPIC);
    this.connection.subscribe(TOPOLOGY_TOPIC, 'std_msgs/msg/String', topologyCallback);
    console.log('[MapManager] Subscribing to map topic:', MAP_TOPIC);
    this.connection.subscribe(MAP_TOPIC, 'nav_msgs/OccupancyGrid', occupancyGridCallback);
  }
  ...
  public publishTopology(connection?: RosbridgeConnection): void {
    ...
    const map = this.getTopologyMap();
    try {
      conn.publish(TOPOLOGY_UPDATE_TOPIC, 'std_msgs/msg/String', { data: JSON.stringify(map) });
    } catch (error) {
