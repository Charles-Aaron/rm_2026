...
export class TF2JS {
  ...
  public initialize(connection: RosbridgeConnection): void {
    ...
    const tfTopic = withRobotNamespace('/tf') ?? '/tf';
    const tfStaticTopic = withRobotNamespace('/tf_static') ?? '/tf_static';
    const tfType = connection.getTopicType(tfTopic) || 'tf2_msgs/TFMessage';
    const tfStaticType = connection.getTopicType(tfStaticTopic) || 'tf2_msgs/TFMessage';

    try {
      connection.subscribe(tfTopic, tfType, tfCallback);
      this.tfUnsubscribe = () => connection.unsubscribe(tfTopic);
    } catch (error) {
      console.error('[TF2JS] Failed to subscribe to /tf:', error);
    }

    try {
      connection.subscribe(tfStaticTopic, tfStaticType, tfStaticCallback);
      this.tfStaticUnsubscribe = () => connection.unsubscribe(tfStaticTopic);
    } catch (error) {
